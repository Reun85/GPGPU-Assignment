// A simple utility file for facilitating the use of OpenCL.
...
inline bool oclCreateContextByRegex(
    cl::Context &context,
    std::regex platform_name = std::regex("nvidia",
  ...
  for (auto platform : platforms) {
    try {
      ...
      if (!std::regex_search(name, platform_name)) continue;
...
inline bool oclCreateContextBy(
    cl::Context &context, std::string platform_name_substring = "",
    cl_device_type device_type = CL_DEVICE_TYPE_GPU) {
  return oclCreateContextByRegex(
      context,
      std::regex(platform_name_substring, std::regex_constants::ECMAScript |
                                              std::regex_constants::icase),
      device_type);
}
...
inline cl_uint oclZeroCopySizeAlignment(cl_uint requiredSize,
                                        cl::Device device) {
  // The following statement rounds requiredSize up to the next
  // CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE-byte boundary
  return requiredSize +
         (~requiredSize + 1) %
...
inline void oclGetTimeStats(cl::Event operation, cl_ulong &device_start,
                            cl_ulong &device_end) {
  operation.wait();
  operation.getProfilingInfo<cl_ulong>(CL_PROFILING_COMMAND_START,
                                       &device_start);
...
inline const char *oclDeviceTypeString(cl_uint devType) {
  switch (devType) {
...
inline void oclPrintError(const cl::Error &error) {
  std::cout << error.what() << "(" << error.err()
            << " == " << oclErrorString(error.err()) << ")" << std::endl;
}
