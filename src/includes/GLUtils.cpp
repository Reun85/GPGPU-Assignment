...
*/
void loadShader(const GLuint loadedShader,
...
void compileShaderFromSource(const GLuint loadedShader,
  ...
  if (GL_FALSE == result || infoLogLength != 0) {
    ...
    SDL_LogMessage(SDL_LOG_CATEGORY_ERROR,
                   ...
                   "[glLinkProgram] Shader compile error: %s",
                   ErrorMessage.data());
  }
}

void AssembleProgram(const GLuint programID,
                     const std::filesystem::path& vs_filename,
                     const std::filesystem::path& fs_filename) {
  //
  // shaderek betöltése
  //
  ...
  if (GL_FALSE == result || infoLogLength != 0) {
    ...
    SDL_LogMessage(SDL_LOG_CATEGORY_ERROR,
                   (result) ? SDL_LOG_PRIORITY_WARN : SDL_LOG_PRIORITY_ERROR,
                   "[glLinkProgram] Shader linking error: %s",
                   ErrorMessage.data());
  }
...
static void invert_image_RGBA(int pitchInPixels, int height,
                              Uint32* image_pixels) {
...
void TextureFromFile(const GLuint tex, const std::filesystem::path& fileName,
                     GLenum Type, GLenum Role) {
  if (tex == 0) {
    SDL_LogMessage(SDL_LOG_CATEGORY_ERROR, SDL_LOG_PRIORITY_ERROR,
                   "Texture object needs to be inited before loading %s !",
  ...
  if (loaded_img == nullptr) {
    SDL_LogMessage(SDL_LOG_CATEGORY_ERROR, SDL_LOG_PRIORITY_ERROR,
                   "[TextureFromFile] Error while loading texture: %s",
                   fileName.string().c_str());
    return;
  }
  ...
  if (Type != GL_TEXTURE_CUBE_MAP && Type != GL_TEXTURE_CUBE_MAP_ARRAY)
    invert_image_RGBA(formattedSurf->pitch / sizeof(Uint32), formattedSurf->h,
                      reinterpret_cast<Uint32*>(formattedSurf->pixels));

  glBindTexture(Type, tex);
  glTexImage2D(
      Role,  // melyik binding point-on van a textúra erőforrás, amihez tárolást
             // rendelünk
...
void SetupTextureSampling(GLenum Target, GLuint textureID,
                          bool generateMipMap) {
  ...
  glTexParameteri(
      ...
      GL_LINEAR);  // bilineáris szürés nagyításkor (ez az alapértelmezett)
  glTexParameteri(
      Target, GL_TEXTURE_MIN_FILTER,
      generateMipMap
          ? GL_LINEAR_MIPMAP_LINEAR
          : GL_LINEAR);  // trilineáris szűrés a mipmap-ekböl kicsinyítéskor
