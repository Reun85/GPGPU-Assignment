...
constexpr unsigned short From2Char(const char ch1, const char ch2) {
  unsigned short sh =
      static_cast<unsigned short>(ch2) << 8 | static_cast<unsigned short>(ch1);
...
triangulatePolygon(const std::vector<glm::vec2> &polygon) {
  ...
  struct {
    ...
    void SetTriangle(unsigned int triIdx, unsigned int i0, unsigned int i1,
                     unsigned int i2) {
  ...
  while (polygonNodes.size() > 2) {
    unsigned int minAngleIdx = static_cast<unsigned int>(std::distance(
        polygonNodes.cbegin(),
    ...
    for (; !edges2check.empty(); edges2check.pop_front()) {
      ...
      bool leftFound =
          triangulation.findTri4Edge(edge2check, leftTriIdx, _idx3);
      unsigned int rightTriIdx, _idx1;
      bool rightFound = triangulation.findTri4Edge(
          {edge2check[1], edge2check[0]}, rightTriIdx, _idx1);
      ...
      if (leftFound && rightFound) {
        ...
        glm::mat3 D(P0mP3.x, P0mP3.y, glm::dot(P0mP3, P0mP3), P1mP3.x, P1mP3.y,
                    glm::dot(P1mP3, P1mP3), P2mP3.x, P2mP3.y,
                    glm::dot(P2mP3, P2mP3));
