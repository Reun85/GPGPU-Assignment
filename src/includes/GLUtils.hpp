...
inline void TextureFromFile(const GLuint tex,
                            const std::filesystem::path& fileName,
                            GLenum Type = GL_TEXTURE_2D) {
  TextureFromFile(tex, fileName, Type, Type);
}
...
template <typename VertexT>
[[nodiscard]] OGLObject CreateGLObjectFromMesh(
    const MeshObject<VertexT>& mesh,
    std::initializer_list<VertexAttributeDescriptor> vertexAttrDescList) {
  OGLObject meshGPU = {0};
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh.indexArray.size() * sizeof(GLuint),
               mesh.indexArray.data(), GL_STATIC_DRAW);
  ...
  for (const auto& vertexAttrDesc : vertexAttrDescList) {
    ...
    glVertexAttribPointer(
        vertexAttrDesc.index,  // a VB-ben található adatok közül a 0. "indexű"
                               // attribútumait állítjuk be
        ...
        reinterpret_cast<const void*>(
            vertexAttrDesc
                .strideInBytes)  // a 0. indexű attribútum hol kezdődik a
                                 // sizeof(Vertex)-nyi területen belül
    );
  }
