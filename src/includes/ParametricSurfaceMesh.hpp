...
[[nodiscard]] MeshObject<Vertex> GetParamSurfMesh(const SurfT& surf,
                                                  const std::size_t N = 80,
                                                  const std::size_t M = 40) {
  MeshObject<Vertex> outputMesh;

  // NxM darab négyszöggel közelítjük a parametrikus felületünket => (N+1)x(M+1)
  // pontban kell kiértékelni
  ...
  // indexpuffer adatai: NxM négyszög = 2xNxM háromszög = háromszöglista esetén
  // 3x2xNxM index
  ...
  for (std::size_t j = 0; j < M; ++j) {
    for (std::size_t i = 0; i < N; ++i) {
      ...
      outputMesh.indexArray[index + 0] =
          static_cast<GLuint>((i) + (j) * (N + 1));
      outputMesh.indexArray[index + 1] =
          static_cast<GLuint>((i + 1) + (j) * (N + 1));
      outputMesh.indexArray[index + 2] =
          static_cast<GLuint>((i) + (j + 1) * (N + 1));
      outputMesh.indexArray[index + 3] =
          static_cast<GLuint>((i + 1) + (j) * (N + 1));
      outputMesh.indexArray[index + 4] =
          static_cast<GLuint>((i + 1) + (j + 1) * (N + 1));
      outputMesh.indexArray[index + 5] =
          static_cast<GLuint>((i) + (j + 1) * (N + 1));
    }
  }
