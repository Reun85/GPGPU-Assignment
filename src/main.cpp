...
int main(int argc, char* args[]) {
  ...
  // a grafikus alrendszert kapcsoljuk csak be, ha gond van, akkor jelezzük és
  // lépjünk ki
  ...
  // Miután az SDL Init lefutott, kilépésnél fusson le az alrendszerek
  // kikapcsolása. Így akkor is lefut, ha valamilyen hiba folytán lépünk ki.
  std::atexit(SDL_Quit);

  //
  // 2. lépés: állítsuk be az OpenGL-es igényeinket, hozzuk létre az ablakunkat,
  // indítsuk el az OpenGL-t
  //

  // 2a: OpenGL indításának konfigurálása, ezt az ablak létrehozása előtt kell
  // megtenni!

  // beállíthatjuk azt, hogy pontosan milyen OpenGL context-et szeretnénk
  // létrehozni - ha nem tesszük, akkor automatikusan a legmagasabb elérhető
  // verziójút kapjuk
  ...
  // állítsuk be, hogy hány biten szeretnénk tárolni a piros, zöld, kék és
  // átlátszatlansági információkat pixelenként
  ...
  win = SDL_CreateWindow(
      "Hello SDL&OpenGL!",  // az ablak fejléce
  ...
  //
  // 3. lépés: hozzunk létre az OpenGL context-et - ezen keresztül fogunk
  // rajzolni
  //
  ...
  if (context == nullptr) {
    SDL_LogError(SDL_LOG_CATEGORY_ERROR,
                 "[OGL context creation] Error during the creation of the OGL "
                 "context: %s",
                 SDL_GetError());
    return 1;
  }
  ...
  SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Running OpenGL %d.%d",
  ...
  if (glVersion[0] == -1 && glVersion[1] == -1) {
    ...
    SDL_LogError(SDL_LOG_CATEGORY_ERROR,
                 "[OGL context creation] Error during the inialization of the "
                 "OGL context! Maybe one of the SDL_GL_SetAttribute(...) calls "
                 "is erroneous.");
